...
The reasons to use WPS 1.0.0 for the implementation were :
...
- **data:** Data reference passed to the application. If null the data referece is defined in the OWC document (code=http://www.opengis.net/spec/owc-atom/1.0/opensearch).

Here is a sample for the ProcessParameters: 
**ProcessParameters:**
```json
{
	"cloud":"ncpu=4,exec_policy=parallel",
...
In the case where a process is executed in parallel no a larger set of files, the response includes a list of JobIDs. This is convenient so that a GetStatus($JobID) operation can be executed on each single child process.
